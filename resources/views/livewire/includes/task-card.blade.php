<div wire:key=" {{ $task->id }}"
    ...
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex items-center">
            ...
            @if ($task->completed)
                <input wire:click="toggle({{ $task->id }})" class="mr-2" type="checkbox" name="" id=""
                    checked>
            @else
                <input wire:click="toggle({{ $task->id }})" class="mr-2" type="checkbox" name=""
                    id="">
            @endif
            ...
            @if ($editingTaskId == $task->id)
                ...
                <div>
                <input wire:model="editingName" type="text" placeholder="Task.."
            ...
            @else
                <h3 class="text-lg text-semibold text-gray-800">{{ $task->name }}</h3>
            @endif
    ...
    <div class="mt-3 text-xs text-gray-700">
            ...
            <button
                wire:click="edit({{$task->id}})" class="mt-3 px-4 py-2 bg-teal-500 text-white font-semibold rounded hover:bg-teal-600">Update</button>
